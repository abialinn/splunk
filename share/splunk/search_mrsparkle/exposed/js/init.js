...
Splunk.namespace('Pony');
Splunk.Pony.ride = function() {
    $.getScript(Splunk.util.make_url('/static/js/contrib/soundmanager2-nodebug-jsmin.js'), function() {
        soundManager.url = Splunk.util.make_url('/static/flash') + '/';
        soundManager.debugMode = false;
        soundManager.flashVersion = 9;
        soundManager.reboot();
        soundManager.onready(function() {
            Splunk.Pony.sndDispatch = soundManager.createSound('lo', Splunk.util.make_url('/static/flash/lo.mp3'));
        });
    });
    $(document).bind('jobDispatched', function() {
        Splunk.Pony.sndDispatch.setPosition(80);
        Splunk.Pony.sndDispatch.play();
    });
};
