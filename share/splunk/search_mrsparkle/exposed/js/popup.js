...
Splunk.Popup.createSchedulePDFForm = function(formContainer, title, pdfService) {
	var pdfUrl,
        viewLabel;
    var getArgs = {};
...
Splunk.Popup.SchedulePDF = function($targetContainer, error) {
	...
	// if pdfgen/is_available can be accessed, use its response as the ultimate determinant of pdfability
	// if pdfgen/is_available cannot be accessed display unavailable dialog
	var params = {};
	params["viewId"] = Splunk.ViewConfig.view.id;
	params["namespace"] = Splunk.ViewConfig.app.id;
	params["owner"] = $C.USERNAME;

	// use __raw decorator to make splunkweb proxy return the https status as its own status
	$.get(Splunk.util.make_full_url("splunkd/__raw/services/pdfgen/is_available", params ))
		.success( function(data, textStatus, jqXHR) { pdfGenAvailability(data, textStatus, jqXHR); })
		.error( function(jqXHR, textStatus, errorThrown) { pdfGenAvailability(null, textStatus, jqXHR); });
    
    function pdfGenAvailability(data, textStatus, jqXHR) {
		if(jqXHR.status===200) {
			switch(data) {
				case "pdfgen":
					displaySchedulePDFForm(data);
					break;
				case "deprecated":
					// if pdfgen/is_available determines that we should use the deprecated server
					// check for the existence of the deprecated server
					checkForDeprecatedServer();
					break;
				case "denied":
    ...
    function checkForDeprecatedServer() {
		$.getJSON(Splunk.util.make_url("report/is_enabled"), function(response, textStatus) {
			switch(response.status) {
				case 'enabled':
					displaySchedulePDFForm("deprecated");
					break;
				case 'disabled':
					displaySchedulePDFDeprecated();
					break;
				case 'notinstalled':
					displaySchedulePDFDeprecated();
					break;
    ...
    function displaySchedulePDFDeprecated() {
        var msg;

		var linkToHelp = Splunk.util.make_full_url("help", {location:"learnmore.pdf.views"});
		msg = _("<p>PDF generation is unavailable for this view. <a href=") + linkToHelp + _(">Learn more.</a></p>");
