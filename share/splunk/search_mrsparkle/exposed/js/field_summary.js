...
Splunk.Popup.FieldSummaryLayer = $.klass({
    ...
    open: function(openerElement, onCancelled) {
        this.popupLayer
            .find('.splHeader h2').text("Loading...")
            .end()
            .css("left",openerElement.offset().left + openerElement.outerWidth())
            .css("top", openerElement.offset().top)
    ...
    updateNumericalStats: function(fieldNode, isNumeric) {
        ...
        if (isNumeric) {
            ...
            for (var i=0; i<numericalStats.length; i++) {
                ...
                $("<span>")
                    .html(sprintf(_("<b>%(label)s</b>: %(value)s"), numericalStats[i]))
                    .appendTo(container);
            }
        }
    },
    updateHeader: function(pageSize, isNumeric) {
        ...
        this.popupLayer.find('.splHeader h2')
            //.text(this.buildCountAndNoun(this.fieldName, this.distinctCount))     
            .text(this.fieldName + suffix);
    ...
    renderTableRowFromFieldNode: function(tbody, fieldNode, onFieldValueClick) {
        ...
        var graphRow = $('<tr></tr>')
            .append(graphHeader) // field name
	        .append($("<td></td>").text(format_number(count))) // field count
	        .append($("<td></td>").text(format_percent(count / this.eventCount))) // percentage
            .append(graphCell);
    ...
    buildReportLinks: function(isNumeric) {
        ...
        var container = $(this.reportLinkSelector, this.popupLayer)
            .html('<h4>' + _('Charts') + ' </h4>');
        ...
        for (var i=0; i<labelSet.length; i++) {
            ...
            var anchor = $('<a></a>')
                .text(label)
                .attr("rel", "popup")        // best practices markup. Might help us with some popup blockers somewhere.
                .attr("href", fallbackURL)   // the URL we initially load, that will be changed when the POST to parse returns.
