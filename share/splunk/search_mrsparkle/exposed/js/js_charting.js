...
Splunk.JSCharting.AbstractChart = $.klass(Splunk.JSCharting.AbstractVisualization, {
    ...
    formatYAxis: function(properties, data) {
        ...
        if(typeof this.hcConfig.yAxis.title.text === 'undefined') {
            if (this.processedData.fieldNames.length === 1) { 
                this.hcConfig.yAxis.title.text = this.processedData.fieldNames[0];
            }else{
                // when dasboard xml does not mention yAxis title
                // and  yAxis title is still undefined,
                // ensure we default to '' to prevent 
                // hcChart using its default value = 'Y-values'. 
                this.hcConfig.yAxis.title.text = '';
            }    
        }    
    },
