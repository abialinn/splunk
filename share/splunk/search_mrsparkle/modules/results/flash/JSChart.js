Splunk.Module.JSChart = $.klass(Splunk.Module.DispatchingModule, {
    ...
    initialize: function($super, container) {
        ...
        Splunk.Legend.addEventListener("labelIndexMapChanged", function() {
            if(this.chart && this.chart.needsLegendMapping) {
...
//                }
    ...
    onChartClicked: function(event) {
        ...
        // if a point index was given, use it to assemble row context
        if(event.hasOwnProperty('pointIndex')) {
            var fieldNames = this.response.fields,
                dataColumns = this.response.columns,
                rowContext = {};

            for(i = 0; i < fieldNames.length; i++) {
                if(/^_time|^[^_]/.test(fieldNames[i])) {
                    rowContext['row.' + fieldNames[i]] = dataColumns[i][event.pointIndex];
                }
            }
            this._selection.rowContext = rowContext;
        }
    ...
    draw: function(updateCount) {
        ...
        if(this.chart.needsLegendMapping) {
    ...
    renderChart: function(callback) {
        ...
        if(this.renderSuccessDfd.isResolved()) {
            this.renderSuccessDfd = $.Deferred();
        }
    ...
    getChartReadyData: function(response, fieldInfo, properties) {
        ...
        var context = this.getContext(),
            truncationLimit = parseInt(context.get('charting.chart.resultTruncationLimit'), 10)
                || parseInt(this.getParam('resultTruncationLimit'), 10) || 0;
    ...
    applyColorsAndDraw: function(fieldList, callback) {
        ...
        // otherwise we have what we need to draw right away
            ...
            this.chart.setColorMapping(this.legendFieldList, this.legendFieldMap, legendTotalSize);
